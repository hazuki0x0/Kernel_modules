...
static int cpufreq_smartmax_boost_task(void *data) {
	...
	while (1) {
		...
		if (lock_policy_rwsem_write_s(0) < 0)
			continue;
	...
	case CPUFREQ_GOV_START:
		...
		if (dbs_enable == 1) {
			if (!boost_task_alive) {
				...
				sched_setscheduler_nocheck_s(boost_task, SCHED_RR, &param);
...
static int __init cpufreq_smartmax_init(void) {
	...
	SYMSEARCH_BIND_FUNCTION_TO(smartmax, nr_running, nr_running_s);
	SYMSEARCH_BIND_FUNCTION_TO(smartmax, unlock_policy_rwsem_write, unlock_policy_rwsem_write_s);
	SYMSEARCH_BIND_FUNCTION_TO(smartmax, lock_policy_rwsem_write, lock_policy_rwsem_write_s);
	SYMSEARCH_BIND_FUNCTION_TO(smartmax, sched_setscheduler_nocheck, sched_setscheduler_nocheck_s);
